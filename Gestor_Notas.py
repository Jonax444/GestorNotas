...
def Mostrar_curso():
    ...
    if cursos:
        ...
        for i, c in enumerate(cursos, start=1):
            estado = "APROBADO" if c['nota'] >= 60 else "REPROBADO"
...
def aprobados_reprobado():
    ...
    # Recorre cada curso y verifica si aprobó o no (60 es la nota mínima)
    for curso in cursos:
        if curso['nota'] >= 60:
            aprobados += 1
        else:
    ...
    for curso in cursos:
        ...
        if curso['nombre'].lower() == nombre_cursos.lower():
            ...
            registrar_cambio(historial_cambios, "ACTUALIZACIÓN DE NOTA",
                             f"Nota del curso '{nombre_cursos}' actualizada de {nota_anterior:.2f} a {nota_nueva:.2f}")
            return True
    return False
...
# Función que elimina un curso de la lista CON CONFIRMACIÓN
def eliminar_curso(cursos, remover):
    ...
    for curso in cursos:
        # Si encuentra el curso, pide confirmación antes de eliminarlo
        if curso['nombre'].lower() == remover.lower():
            print(f"\nCurso encontrado: {curso['nombre']} - Nota: {curso['nota']:.2f}")
            confirmacion = input(f"¿Está seguro que desea eliminar '{curso['nombre']}'? (S/N): ").upper().strip()

            if confirmacion in ["SI", "S"]:
                # Registra el cambio en el historial ANTES de eliminar
                registrar_cambio(historial_cambios, "ELIMINACIÓN DE CURSO",
                                 f"Curso '{curso['nombre']}' con nota {curso['nota']:.2f} fue eliminado")
                cursos.remove(curso)
                print(f"\n✓ Curso '{curso['nombre']}' eliminado correctamente.\n")
                return True
            else:
                print("\n✗ Operación cancelada. El curso NO fue eliminado.\n")
                return False
    else:
...
def busqueda_binaria(cursos, busqueda):
    ...
    while izquierda <= derecha:
        ...
        if nombre_medio == busqueda_lower:
            estado = "APROBADO" if curso_medio['nota'] >= 60 else "REPROBADO"
...
# ========== FUNCIONES PARA COLA DE REVISIONES (SIN USAR DEQUE) ==========
# La cola usa estructura FIFO (First In, First Out - primero en entrar, primero en salir)

def simular_cola_revision(cursos):
    """
    Simulación simple de cola de revisiones sin usar deque.
    Usa una lista normal de Python con pop(0) para simular FIFO.
    """
    print("\n" + "=" * 50)
    print("SIMULACIÓN DE COLA DE SOLICITUDES DE REVISIÓN")
    print("=" * 50)

    # Verifica que haya cursos registrados
    if not cursos:
        print("\n⚠ No hay cursos registrados. Primero debe registrar cursos (opción 1).\n")
        return
    ...
    cola_revision = []  # Lista normal en lugar de deque

    print("Ingrese los nombres de los cursos para enviar a revisión")
    print("(Escriba 'fin' cuando termine de ingresar cursos)\n")

    # Fase 1: Agregar cursos a la cola con validación
    while True:
        nombre_curso = input("> ").strip()
        ...
        # Verifica si el curso existe en la lista de cursos registrados
        curso_encontrado = False
        for curso in cursos:
            if curso['nombre'].lower() == nombre_curso.lower():
                curso_encontrado = True
                cola_revision.append(curso['nombre'])  # Agrega al final de la lista
                print(f"  ✓ '{curso['nombre']}' agregado a la cola de revisión")
                break

        if not curso_encontrado:
            print(f"  ✗ El curso '{nombre_curso}' no está registrado. Intente con otro nombre.")

    # Fase 2: Procesar la cola
    if not cola_revision:
        print("\nNo se ingresaron cursos válidos para revisión")
        return
    ...
    # Procesa cada curso en orden de ingreso (FIFO)
    # pop(0) quita el primer elemento de la lista (el más antiguo)
    while cola_revision:
        curso = cola_revision.pop(0)  # Quita del inicio (FIFO)
...
while repetir:
    ...
    # OPCIÓN 5: Buscar un curso revisando toda la lista (búsqueda lineal)
        ...
        while True:
            ...
            busqueda = input("Ingrese el nombre del curso a buscar: ").strip()
    ...
    # OPCIÓN 7: Eliminar un curso de la lista
        ...
        while True:
            ...
            eliminar_curso(cursos, None)

            if cursos:  # Solo muestra si aún hay cursos
                Mostrar_curso()
    ...
    # IMPORTANTE: Solo funciona si los cursos están ordenados alfabéticamente
        ...
        while True:
            ...
            # Muestra la lista de cursos disponibles
            print("\nCursos registrados (ordenados alfabéticamente):")
            Mostrar_curso()
    ...
    # OPCIÓN 11: Simulación de cola de revisiones (SIN deque, usando pop(0))
    elif opcion == 11:
        simular_cola_revision(cursos)
